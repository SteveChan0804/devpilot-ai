...
const toolNames = ["list_files", "read_file", "search_code", "get_git_status", "write_file", "run_command"];
...
export async function agentRoutes(app) {
    ...
    app.post("/agent/run", async (request, reply) => {
        ...
        try {
            const result = await runAgentTask(parsed.data.repositoryId, parsed.data.task, parsed.data.provider);
    ...
    app.post("/agent/tools/execute", async (request, reply) => {
        ...
        if (isApprovalTool(tool))
            return { status: "approval_required", approvalId: await createApproval({ repositoryId, tool, args }), tool, args };
    ...
    app.post("/agent/approve", async (request, reply) => {
        ...
        return { status: "completed", tool: action.tool, result: await executeTool(action.tool, await getRepositoryRoot(action.repositoryId), action.args) };
    });
}
