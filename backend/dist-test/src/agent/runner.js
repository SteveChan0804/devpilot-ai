import { z } from "zod";
import { completeChat } from "../services/llm.service.js";
import { retrieveChunks } from "../services/search.service.js";
import { executeTool, isApprovalTool, toolDefinitions } from "./tools.js";
...
function fallbackSearchQuery(task) {
...
export async function runAgentTask(repositoryId, task, provider) {
    ...
    if (!plan)
        return { status: "needs_clarification", answer: plannerResponse, sources, calls: [] };
    const results = [];
    for (const call of plan.calls) {
        try {
            const args = { ...call.args };
            if (call.tool === "search_code" && !args.query)
                args.query = fallbackSearchQuery(task);
            if (isApprovalTool(call.tool))
                results.push({ tool: call.tool, status: "approval_required", approvalId: await createApproval({ repositoryId, tool: call.tool, args }) });
            else
                results.push({ tool: call.tool, status: "completed", result: await executeTool(call.tool, repositoryRoot, args) });
        }
        catch (error) {
    ...
    if (!hasUsefulEvidence && /\b(where|how|explain|find|which)\b/i.test(task)) {
        try {
            results.push({ tool: "search_code", status: "completed", result: await executeTool("search_code", repositoryRoot, { query: fallbackSearchQuery(task) }) });
        }
        catch (error) {
            results.push({ tool: "search_code", status: "failed", error: error instanceof Error ? error.message : String(error) });
        }
    }
    if (results.some((result) => result.status === "approval_required"))
        return { status: "approval_required", answer: "I need your approval before I can perform the requested changes or commands.", sources, calls: results };
    if (hasMutationIntent(task))
        return { status: "approval_required", answer: "This task requests a change or command, but the planner did not produce a safe actionable proposal. No changes were made. Please review the requested operation and try again.", sources, calls: results };
