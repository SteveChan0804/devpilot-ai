...
export async function resolveApproval(id: string, approved: boolean) {
  ...
  .returning({ id: agentApprovals.id, taskId: agentApprovals.taskId, repositoryId: agentApprovals.repositoryId, tool: agentApprovals.tool, args: agentApprovals.args });
  return rows[0] as { id: string; taskId: string | null; repositoryId: string; tool: AgentTool; args: ToolArgs } | undefined;
}
