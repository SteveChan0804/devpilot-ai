...
export async function runIndexing(repositoryId: string, rootPath: string, jobId?: string) {
  recordMetric("indexing.started");
  const updateJob = async (values: Partial<typeof indexJobs["$inferInsert"]>) => {
    if (jobId) await db.update(indexJobs).set(values).where(eq(indexJobs.id, jobId));
  };
