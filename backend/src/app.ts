...
export function buildApp() {
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    if (request.method === "OPTIONS" || request.url === "/health" || request.url === "/ready") return;
    if (env.API_KEY) {
      const provided = request.headers["x-api-key"] ?? request.headers.authorization?.replace(/^Bearer\s+/i, "");
      if (provided !== env.API_KEY) return reply.code(401).send({ error: "Authentication required", requestId: request.id });
    }
    const now = Date.now();
    const current = rateLimits.get(request.ip);
    if (!current || current.resetAt <= now) rateLimits.set(request.ip, { count: 1, resetAt: now + 60_000 });
    else {
      current.count++;
      if (current.count > env.RATE_LIMIT_PER_MINUTE) return reply.code(429).header("retry-after", Math.ceil((current.resetAt - now) / 1000)).send({ error: "Rate limit exceeded", requestId: request.id });
    }
  });
  ...
  app.get("/ready", async (_request, reply) => {
    try {
      ...
      if (!ollama.ok) throw new Error(`Ollama returned ${ollama.status}`);
